...
abstract contract DexRegistry is MultiCallable {
    ...
    function initDexRegistry(address[] memory _dexList, string[] memory _funcs, address[] memory _codecs) internal onlyInitializing {
        _setDexCodecs(_dexList, _funcs, _codecs);
    }

    function setDexCodecs(address[] memory _dexList, string[] memory _funcs, address[] memory _codecs) external onlyOwnerMulticall {
        _setDexCodecs(_dexList, _funcs, _codecs);
    }

    function _setDexCodecs(address[] memory _dexList, string[] memory _funcs, address[] memory _codecs) private {
    ...
    function _setDexCodec(address _dex, bytes4 _selector, address _codec) private {
    ...
    function getCodec(address _dex, bytes4 _selector) external view returns (ICodec) {
